...
def main():
    ...
    #############################################################################################################     
        ...
        path_to_U87_2 = os.path.join('data_ik_may2022/U87_1_after_surgery/')
        #path_to_U87_3 = os.path.join('data_ik_may2022/U87_3/')            
        #path_to_U87_4 = os.path.join('data_ik_may2022/U87_4/')            
        path_to_control_1 = os.path.join('data_ik_may2022/Control/')
        path_to_control_2 = os.path.join('data_ik_may2022/Control_anna/')
        #path_to_control_3 = os.path.join('data_ik_may2022/Control_3/')
        #path_to_control_4 = os.path.join('data_ik_may2022/Control_3/')
    ...
    #############################################################################################################    
        ...
        for fn in file_list:
            with open(os.path.join(path_to_U87_2, fn), encoding='utf-8-sig') as ref:
                #result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
                result = np.array(list(csv.reader(ref, delimiter=';'))).astype("float")
                U87_2.append(result[:,1])
        file_names.append(file_list)
        # # U87_3
        # file_list = os.listdir(path_to_U87_3)
        # for fn in file_list:
        #     with open(os.path.join(path_to_U87_3, fn), encoding='utf-8-sig') as ref:
        #         #result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
        #         result = np.array(list(csv.reader(ref, delimiter='\t'))).astype("float")
        #         frequencies.append(result[:,0])
        #         U87_3.append(result[:,1])

        # # U87_4
        # file_list = os.listdir(path_to_U87_4)
        # for fn in file_list:
        #     with open(os.path.join(path_to_U87_4, fn), encoding='utf-8-sig') as ref:
        #         #result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
        #         result = np.array(list(csv.reader(ref, delimiter='\t'))).astype("float")
        #         frequencies.append(result[:,0])
        #         U87_4.append(result[:,1])

        # Control_1
        file_list = os.listdir(path_to_control_1)
        for fn in file_list:
            with open(os.path.join(path_to_control_1, fn), encoding='utf-8-sig') as ref:
                result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
        ...
        for fn in file_list:
            with open(os.path.join(path_to_control_2, fn), encoding='utf-8-sig') as ref:
                result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
                control_2.append(result[:,1])       
        file_names.append(file_list)

        names = frequencies[0][lower_bound:upper_bound]
        frequencies = names
        #control_2 = np.copy(control_1)
        #control_3 = np.copy(control_1)
...
# ######################################################        
    ...
    #############################################################################################################    
        if 1:
            ...
            X = np.zeros((len(group1)+len(group2),upper_bound - lower_bound))            
            use_pca = 1
            use_normalization = 1
            if use_pca:
                print("PCA is used")
            ...
            if group1_name == "U87_1":
                for sample in U87_1:
                    X[i, :] = sample[lower_bound:upper_bound]
                    i+=1
                    labels.append('Tumor before surgery')
                    y.append(k)
                k+=1
            ...
            if group2_name == "U87_2":
                for sample in U87_2:
                    X[i, :] = sample[lower_bound:upper_bound]
                    i+=1
                    labels.append('Tumor after surgery')
                    y.append(k)
                k+=1
...
############## Mean and median spectra #############################
        if 1:        
            ...
            plt.legend()
            tmp_ticks = np.arange(400, 4000, 200) 
            plt.xticks(tmp_ticks, tmp_ticks) 
        ...
        ###### Plot errorbar###############
            if 1:
                #### 1 week #######
                plt.errorbar(frequencies, np.mean(X[:len(group1)],axis=0), yerr=np.std(X[:len(group1)],axis=0),color='green',label="mean Tumor",alpha = 0.7)
                plt.errorbar(frequencies, np.mean(X[len(group1):],axis=0), yerr=np.std(X[len(group1):],axis=0),color='red',label="mean Control",alpha = 0.5)
                plt.plot(frequencies, np.mean(X[len(group1):],axis=0), color="red")
                plt.legend()
                tmp_ticks = np.arange(800, 4000, 200) 
                plt.xticks(tmp_ticks, tmp_ticks) 
                plt.xlabel('Wavenumber, cm$^{-1}$', **axis_font)   
...
###################################################################
        ...
        plt.tight_layout()
        plt.savefig('ir_fig_3.png', dpi = 600)
        plt.close()     
...
# Savitsky-Golay filter            
    ...
    #        plt.plot(ind, medfilt(loadings[1,:], 1),".",color="green",label="Loadings for PC1_1")
        ...
        #    plt.plot(ind, median_loadings_il14,"-",color="blue",label="median_loadings_il14")
            ...
            plt.tight_layout()
            #plt.show()
            plt.savefig('ir_fig_loadings_not_normed.png', dpi = 600)
            #plt.show()
            plt.close()             
...
# Savitsky-Golay filter            
    ...
    #        plt.plot(ind, medfilt(loadings[1,:], 1),".",color="green",label="Loadings for PC1_1")
        ...
        #    plt.plot(ind, median_loadings_il14,"-",color="blue",label="median_loadings_il14")
            ...
            plt.legend() 
            plt.tight_layout()
            plt.savefig('ir_fig_loadings_normed.png', dpi = 600)
            #plt.show()
            plt.close()      
    #        return 0    
    ##########################################################################################################################
    ...
    ##########################################################################################################################  
        if 0:        
            ...
            plt.plot(features, l)
            plt.tight_layout()
            plt.savefig('ir_fig_2_num_of_PC.png', dpi = 600)
            plt.close()
            #plt.show()
            # return 1
...
#                items_to_skip += len(U87_2)
        ...
        n_components = 10
        vis_folder_name = 'ir_visualization2d_' + str(n_components)
        ...
        for i in range(0, n_components):
            for j in range(i+1, n_components):
                ...
                for group in splitted:
                    ...
                    for ii, txt in enumerate(group[0][:, i]):
                        ax.annotate(str(ii), (group[0][ii, i], group[0][ii, j]))
                ax.legend()
                ax.grid(False)
    ...
    if 0:
        ...
        plt.plot(np.array(features), importance)
        tmp_ticks = np.arange(800, 4000, 200) 
        plt.xticks(tmp_ticks, tmp_ticks) 
        plt.xlabel('Wavenumber, cm$^{-1}$', **axis_font)   
        plt.ylabel('Lasso regression coefficients,\n Arb. Units', **axis_font)    
        plt.tight_layout()         
        plt.savefig('fig_ir_importance.png', dpi = 600)
        plt.close()        
        print('done')   
...
# Run classifier with cross-validation and plot ROC curves
    ...
    #                            name='_nolegend_',
            ...
            precs.append(precision_score(y_sub[test], classifier.predict(X_sub[test]),  average='weighted', zero_division=0))
            recs.append(recall_score(y_sub[test], classifier.predict(X_sub[test]),  average='weighted', zero_division=0))
            confusion_matrices.append(confusion_matrix(y_sub[test], classifier.predict(X_sub[test]), labels=[0, 1]))
            c_reps.append(classification_report(y_sub[test], classifier.predict(X_sub[test]), target_names=[group1_name, group2_name], zero_division=0))
