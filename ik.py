...
def main():
    ...
    #############################################################################################################    
        ...
        for fn in file_list:
            with open(os.path.join(path_to_control_3, fn), encoding='utf-8-sig') as ref:
                #result = np.array(list(csv.reader(ref, delimiter=";"))).astype("float")
                result = np.array(list(csv.reader(ref, delimiter='\t'))).astype("float")
...
# ######################################################        
        if 1:
            ...
            for i in range(0,len(control_3)):
                if yhat[i]==-1:                    
                    print("Outliers are:",i)            
    ...
    #############################################################################################################    
        if 1:
            ...
            #X = np.zeros((len(U87_1) + len(control_2) ,len(U87_2[0])))
