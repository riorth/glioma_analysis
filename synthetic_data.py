...
def main():
    ...
    #############################################################################################################
        # data analysis
    #############################################################################################################     
        path_to_data = os.path.join('synthetic/')
    ...
    #############################################################################################################    
        # class_1
        X = pd.read_csv(path_to_data+'Loadings_with_noise.csv', sep=';')         
        pc1 = np.asarray(X['PC1'].astype(float))
        pc3 = np.asarray(X['PC3'].astype(float))
        print ('done')
...
# Savitsky-Golay filter  
        if 1:          
            ...
            polyorder = 2
            deriv = 0
            plt.plot(savgol_filter(pc1, windows_size, polyorder, deriv),"-",color="blue",label="Loadings for PC1")
    #        plt.plot(ind, medfilt(loadings[1,:], 1),".",color="green",label="Loadings for PC1_1")
        #    plt.plot(ind, median_loadings_h,"--",color="black",label="median_loadings_h")
            plt.plot(savgol_filter(pc3, windows_size, polyorder, deriv),"-",color="purple",label="Loadings for PC3")
        #    plt.plot(ind, median_loadings_il14,"-",color="blue",label="median_loadings_il14")
            ...
            plt.xticks(np.arange(0, 10100, 100)) 
    #        plt.yticks(np.arange(0, 11, 1)) 
            plt.xlabel('Frequencies, cm$^{-1}$', **axis_font)
            plt.ylabel('PCA Loadings values, a.u.', **axis_font)   
            plt.legend() 
            plt.tight_layout()
            plt.savefig('fig_loadings_synthetic.png', dpi = 600)
            plt.show()
            plt.close()             
...
###################################################################
        ...
        if 0:
            ...
            plt.ylabel('THz intensities, Arb. Units', **axis_font)  
            plt.tight_layout()           
            plt.show()
            plt.close()

            #### 2 week #######
            plt.errorbar(frequencies, np.mean(X[len(U87_1):u87_2len],axis=0), yerr=np.std(X[len(U87_1):u87_2len],axis=0),label="U87_2",alpha = 0.9)
            plt.errorbar(frequencies, np.mean(X[u87_4len+len(control_1):u87_4len+ctrl_2len],axis=0), yerr=np.std(X[u87_4len+len(control_1):u87_4len+ctrl_2len],axis=0),label="Control_2",alpha = 0.1)
            plt.plot(frequencies, np.mean(X[u87_4len+len(control_1):u87_4len+ctrl_2len],axis=0), label="Control_2", color="red")
            plt.legend()
            plt.xlabel('THz frequencies, cm$^{-1}$', **axis_font)
            plt.ylabel('THz intensities, Arb. Units', **axis_font)  
            plt.tight_layout()           
            ...
            #### 3 week ######
            plt.errorbar(frequencies, np.mean(X[u87_2len:u87_3len],axis=0), yerr=np.std(X[u87_2len:u87_3len],axis=0),label="U87_3",alpha = 0.9)
            plt.errorbar(frequencies, np.mean(X[u87_4len+ctrl_2len:u87_4len+ctrl_3len],axis=0), yerr=np.std(X[u87_4len+ctrl_2len:u87_4len+ctrl_3len],axis=0),label="Control_3",alpha = 0.1)
            plt.plot(frequencies, np.mean(X[u87_4len+ctrl_2len:u87_4len+ctrl_3len],axis=0), label="Control_3", color="red")
            plt.legend()
            ...
            #### 4 week #######
            plt.errorbar(frequencies, np.mean(X[u87_3len:u87_4len],axis=0), yerr=np.std(X[u87_3len:u87_4len],axis=0),label="U87_4",alpha = 0.9)
            plt.errorbar(frequencies, np.mean(X[u87_4len+ctrl_3len:u87_4len+ctrl_4len],axis=0), yerr=np.std(X[u87_4len+ctrl_3len:u87_4len+ctrl_4len],axis=0),label="Control_4",alpha = 0.1)
            plt.plot(frequencies, np.mean(X[u87_4len+ctrl_3len:u87_4len+ctrl_4len],axis=0), label="Control_4", color="red")
            plt.legend()
            plt.xlabel('Raman shift, cm$^{-1}$', **axis_font)
            plt.ylabel('Raman intensities, Arb. Units', **axis_font)   
            plt.tight_layout()           
