...
# ######################################################        
    ...
    #############################################################################################################    
        if 1:
            ...
            #X = np.zeros((len(U87_3) + len(control_3) ,len(U87_3[0])))
            #X = np.zeros((len(U87_1)+len(U87_4),len(U87_3[0])))
            #X = np.zeros((len(U87_3),len(U87_3[0])))
            i = 0
            j = 0
            k=0
            if 1:
            ...
            bir = np.array([[500.,2000.],[2500.,3300.]]) 
            for i in range(len(X)):
                ...
                ycalc_arpls, base_arpsl = rp.baseline(frequencies[0], X[i,:], bir,'arPLS',lam=10**5,ratio=0.001)
        ...
        if 1:
            ...
            # week 1
            y_sub_1 = np.array([2.7, 2.7, 2.7, 2.7, 2.7, 2.7, 2.7, 1.0, 1.0, 1.0, 
                            2.9, 2.9, 2.9, 1.4, 1.4, 1.4, 0.7, 0.7, 0.7, 
                            2.9, 2.9, 2.9, 2.9, 2.9, 2.9, 3.9, 3.9, 3.9, 
                            2.9, 2.9, 2.9, 5.1, 5.1, 5.1])

            # week 2
            y_sub_2 = np.array([9.7, 9.7, 9.7, 10.0, 10.0, 10.0, 6.9, 6.9, 6.9, 
                            5.8, 5.8, 5.8, 15.4, 15.4, 15.4, 6.4, 6.4, 6.4, 
                            13.5, 13.5, 13.5, 11.7, 11.7, 11.7, 3.2, 3.2, 3.2, 
                            23.0, 23.0, 23.0])
            # week 3
            y_sub_3 = np.array([68.1, 68.1, 68.1, 21.0, 21.0, 21.0, 139.2, 139.2, 139.2, 139.2, 
                            101.9, 101.9, 101.9, 141.8, 141.8, 141.8, 75.8, 75.8, 75.8, 
                            60.1, 60.1, 60.1, 107.9, 107.9, 107.9, 86.5, 86.5, 86.5])
            # week 4
            y_sub_4 = np.array([96.8, 96.8, 96.8, 51.9, 51.9, 51.9, 117.4, 117.4, 117.4, 110.9, 110.9, 110.9])
            # all
            y_sub = np.hstack([y_sub_1, y_sub_2, y_sub_3, y_sub_4]) 
            ...
            print(np.array(features[0])[importance > 1])
            print(model.predict(X_sub[0].reshape(1,-1)))
            plt.plot(np.array(features[0]), importance)
            plt.legend()
            plt.xlabel('Raman shift, cm$^{-1}$', **axis_font)
            plt.ylabel('Lasso regression coefficients values, Arb. Units', **axis_font)    
            plt.tight_layout()   
            plt.savefig('fig_lasso_raman_importance.png', dpi = 600)
            plt.close()            
            print('done')   
...
############## Mean and median spectra #############################
        if 1:        
            ...
            #plt.plot(np.mean(X[len(U87_1):u87_2len], axis=0),color='blue', label="mean U87_2")
            #plt.plot(np.mean(X[u87_2len:u87_3len], axis=0),color='red', label="mean U87_3")
            plt.plot(np.mean(X[:len(U87_3)], axis=0),color='red', label="mean Tumor")
            plt.plot(np.mean(X[len(U87_3):], axis=0),color='blue', label="mean Control")
            ...
            plt.plot(np.median(X[:len(U87_3)], axis=0),color='red', label="median Tumor")
            plt.plot(np.median(X[len(U87_3):], axis=0),color='blue', label="median Control")
            ...
            plt.tight_layout()
            #plt.show()
            plt.savefig('fig_1_median_spectra.png', dpi = 600)
            plt.close()
...
# Savitsky-Golay filter            
    ...
    #        plt.yticks(np.arange(0, 11, 1)) 
            ...
            plt.legend() 
            plt.tight_layout()
            plt.savefig('fig_loadings_not_normed.png', dpi = 600)
            plt.show()
            plt.close()             
# Savitsky-Golay filter            
            windows_size = 51
    ...
    #        plt.yticks(np.arange(0, 11, 1)) 
            ...
            plt.legend() 
            plt.tight_layout()
            plt.savefig('fig_loadings_normed.png', dpi = 600)
            plt.show()
            plt.close()      
    #        return 0    
    ##########################################################################################################################
    ...
    ##########################################################################################################################  
        if 0:        
            ...
            plt.plot(features, l)
            plt.tight_layout()
            plt.savefig('fig_2_num_of_PC.png', dpi = 600)
            plt.close()
            #plt.show()
            # return 1
...
#                items_to_skip += len(U87_2)
        ...
        n_components = 4
        vis_folder_name = 'new_raman_visualization2d_' + str(n_components)
...
####### ROC for random forest################
    if 1:
        num_features = len(frequencies[0][400:3200])
        features = frequencies[0][400:3200]
        #bound = len(U87_1) + len(control_1)
        #X_sub = XPCAreduced[:bound]
        #y_sub = np.asarray(y[:bound])
        X_sub = np.copy(X[:,400:3200])
        y_sub = np.copy(y)
